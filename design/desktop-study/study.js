...
(function () {
    ...
    function composeGarden() {
        ...
        // Desktop study only: title, garden and page content share one frame.
        // Current and mobile retain the production geometry above.
        var desktopConcept = !narrow && document.documentElement.dataset.desktop === 'concept';
        if (desktopConcept) {
        ...
        if (rule && maxFlowers > 0) {
            ...
            Array.prototype.forEach.call(gardenLine.querySelectorAll('text'), function (label) {
                ...
                label.setAttribute('x', mobile ? left.x : right.x + (desktopConcept ? 12 / matrix.a : 0));
                label.setAttribute('y', threshold - 6 / matrix.a);
                label.setAttribute('text-anchor', mobile || desktopConcept ? 'start' : 'end');
            });
        }
    }
    ...
    function fitGarden() {
        ...
        var desktopConcept = window.innerWidth > 760 && document.documentElement.dataset.desktop === 'concept';
        garden.setAttribute('preserveAspectRatio', desktopConcept ? 'xMidYMax slice' : 'xMidYMax meet');
